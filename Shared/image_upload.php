...
function UploadImage($file, $mode = "admin")
{
    $uploadFileError = "";
    $uploadFileSuccess = "";
    $uploadedFile = "";
    ...
    if (isset($file) && $file['error'] == 0) {
        ...
        if ($fileSize > $maxSize) {
            return ['status' => 'error', 'msg' => "File size cannot exceed 5 MBs!"];
        }

        if (in_array($fileType, $allowedFormats)) {
            $file_exists = isFileAlreadyExist($filePath . $fileName);
            if ($file_exists) {
                return ['status' => 'error', 'msg' => "Image with name \"" . $fileName . "\" already exist!"];
            } else {
        ...
        return ['status' => 'error', 'msg' => "Please upload an image file!"];
    }
}